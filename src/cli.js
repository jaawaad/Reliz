...
async function run() {
  try {
    ...
    if (argv.changelog) {
      ...
      const commits = getNewCommits(latestTag);
      const context = {
        cwd,
        config,
        commits,
    ...
    if (!noIncrement) {
      if (!bump) {
        bump = await selectBumpType(currentVersion, suggested, 'patch', isCi);
      }
      if (bump === 'prerelease') {
        if (!preId && !isCi) {
          preId = await selectPreId(isCi);
        }
        bump = 'patch';
      }
    }
    ...
    if (config.github?.release) {
      try {
        context.releaseUrl = await createGitHubRelease(cwd, tagName, releaseName, releaseBody, config.github, false);
      } catch (err) {
        console.warn('GitHub release failed:', err.message);
      }
    }

    if (config.gitlab?.release) {
      try {
        const url = await createGitLabRelease(cwd, tagName, releaseName, releaseBody, config.gitlab, false);
        if (url) context.releaseUrl = context.releaseUrl || url;
      } catch (err) {
        console.warn('GitLab release failed:', err.message);
      }
    }

    await runPluginLifecycleAsync(plugins, 'release', context);
